...
class RegisterApplicationTeamControllerSpec extends SpecBase with MockitoSugar with TeamGenerator {
  ...
  "RegisterApplicationTeam Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(team.id), NormalMode, ???)(request, messages(application)).toString
      }
    }
