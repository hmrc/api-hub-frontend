  ...
  "ApplicationDetails Controller" - {
      ...
      forAll(teamMemberAndSupporterTable) {
        (user: UserModel) =>
          ...
          running(fixture.playApplication) {
            ...
            contentAsString(result) mustBe view(FakeApplication, Seq.empty, 0, Some(user))(request, messages(fixture.playApplication)).toString
    ...
    "must return OK and the correct view for a GET when the application has a global team" in {
      ...
      running(fixture.playApplication) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustBe view(application, Seq.empty, 0, Some(FakeUser))(request, messages(fixture.playApplication)).toString
        contentAsString(result) must validateAsHtml
      }
    }
    ...
    "must sort the application's team members alphabetically" in {
      ...
      running(fixture.playApplication) {
        ...
        contentAsString(result) mustBe view(expected, Seq.empty, 0, Some(FakeUser))(request, messages(fixture.playApplication)).toString
      }
    }
    ...
    "must return the correct view when the application has APIs added" in {
      ...
      running(fixture.playApplication) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustBe view(application, applicationApis, 0, Some(FakeUser))(request, messages(fixture.playApplication)).toString
        contentAsString(result) must validateAsHtml
      }
    }
