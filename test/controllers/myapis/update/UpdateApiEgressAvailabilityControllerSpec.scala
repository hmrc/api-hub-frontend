...
class UpdateApiEgressAvailabilityControllerSpec extends SpecBase with MockitoSugar with TestHelpers {
  ...
  lazy val updateApiEgressAvailabilityRoute = controllers.myapis.update.routes.UpdateApiEgressAvailabilityController.onPageLoad(NormalMode).url
  val guidesUrl = "http://localhost:8490/guides/integration-hub-guide"

  "UpdateApiEgressAvailability Controller" - {

    "must return OK and the correct view for a GET" in {
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()

        running(application) {
          val request = FakeRequest(GET, updateApiEgressAvailabilityRoute)

          val result = route(application, request).value

          val view = application.injector.instanceOf[ProduceApiEgressAvailabilityView]
          val viewModel = ProduceApiEgressAvailabilityViewModel(guidesUrl, controllers.myapis.update.routes.UpdateApiEgressAvailabilityController.onSubmit(NormalMode))
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
        val userAnswers = UserAnswers(userAnswersId).set(UpdateApiEgressAvailabilityPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request = FakeRequest(POST, updateApiEgressAvailabilityRoute).withFormUrlEncodedBody(("value", "true"))
        val result = route(application, request).value
    ...
    "must clear previous egress answer if user selects 'No'" in {
      val mockSessionRepository = mock[UpdateApiSessionRepository]
      val userAnswersWithEgress = emptyUserAnswers.set(UpdateApiEgressSelectionPage, "my egress").success.value
      ...
      running(application) {
        val request = FakeRequest(POST, updateApiEgressAvailabilityRoute).withFormUrlEncodedBody(("value", "false"))
        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER

        verify(mockSessionRepository).set(argThat { (userAnswers: UserAnswers) =>
          userAnswers.get(UpdateApiEgressSelectionPage).isEmpty
        })
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request = FakeRequest(POST, updateApiEgressAvailabilityRoute).withFormUrlEncodedBody(("value", ""))
    ...
    "must redirect to Journey Recovery for a POST if no existing data is found" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, updateApiEgressAvailabilityRoute)
