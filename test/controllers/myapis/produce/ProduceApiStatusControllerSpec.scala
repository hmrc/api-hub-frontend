...
class ProduceApiStatusControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val produceApiStatusRoute = produceApiRoutes.ProduceApiStatusController.onPageLoad(NormalMode).url

  val formProvider = new ProduceApiStatusFormProvider()
  ...
  "ProduceApiStatus Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(Alpha), NormalMode, FakeUser)(request, messages(application)).toString
      }
    }
