...
class ProduceApiHodControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val produceApiHodRoute = controllers.myapis.produce.routes.ProduceApiHodController.onPageLoad(NormalMode).url
  ...
  "ProduceApiHod Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, NormalMode, FakeUser, hods)(request, messages(application)).toString
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(hods.hods.map(_.code).toSet), NormalMode, FakeUser, hods)(request, messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, NormalMode, FakeUser, hods)(request, messages(application)).toString
      }
    }
