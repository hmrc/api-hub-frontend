...
class ProduceApiHowToAddWiremockControllerSpec extends SpecBase with MockitoSugar {

  private def onwardRoute = Call("GET", "/foo")

  private lazy val produceApiHowToAddWiremockRoute = controllers.myapis.produce.routes.ProduceApiHowToAddWiremockController.onPageLoad(NormalMode).url

  private val formProvider = new ProduceApiHowToAddWiremockFormProvider()
  private val form = formProvider()
  private def viewModel(mode: Mode) =
    ProduceApiHowToAddWiremockViewModel(
      controllers.myapis.produce.routes.ProduceApiHowToAddWiremockController.onSubmit(mode)
    )

  "ProduceApiHowToAddWiremock Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, produceApiHowToAddWiremockRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ProduceApiHowToAddWiremockPage, ProduceApiHowToAddWiremock.values.head).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, produceApiHowToAddWiremockRoute)
