...
import views.html.admin.ConfigurationView

class ConfigurationControllerSpec
  extends SpecBase
    with MockitoSugar
    with TestHelpers
    with HtmlValidation {
    ...
    "must return Ok and the correct view for a support user" in {
      forAll(usersWhoCanSupport) { (user: UserModel) =>
        ...
        running(fixture.application) {
          val request = FakeRequest(controllers.admin.routes.ConfigurationController.onPageLoad())
