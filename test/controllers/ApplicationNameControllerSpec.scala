...
class ApplicationNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ApplicationName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ApplicationNamePage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, applicationNameRoute)
