  ...
  "GET" - {
    ...
    "must return the apis in case-insensitive alphabetical order" in {
      ...
      running(fixture.application) {
        ...
        val zebras = ApiDetail("id1", "ref1", "zebras", "zebras api", "1.0.0", Seq.empty, None, "oas", Live, reviewedDate = Instant.now(), platform = platform, lastUpdated = Instant.now(), maintainer = maintainer).toApiDetailSummary
        val molluscs = ApiDetail("id2", "ref2", "MOLLUSCS", "molluscs api", "1.0.0", Seq.empty, None, "oas", Live, reviewedDate = Instant.now(), platform = platform, lastUpdated = Instant.now(), maintainer = maintainer).toApiDetailSummary
        val aardvarks = ApiDetail("id3", "ref3", "aardvarks", "aardvarks api", "1.0.0", Seq.empty, None, "oas", Live, reviewedDate = Instant.now(), platform = platform, lastUpdated = Instant.now(), maintainer = maintainer).toApiDetailSummary
        val pigeons = ApiDetail("id4", "ref4", "PIGEONS", "pigeons api", "1.0.0", Seq.empty, None, "oas", Live, reviewedDate = Instant.now(), platform = platform, lastUpdated = Instant.now(), maintainer = maintainer).toApiDetailSummary
