...
object UpdateApiEgressPrefixesSummary  {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    val value = answers.get(UpdateApiEgressPrefixesPage) match {
    ...
    val changeUrl = answers.get(UpdateApiEgressPrefixesPage) match {
      case Some(_) => controllers.myapis.update.routes.UpdateApiEgressPrefixesController.onPageLoad(CheckMode).url
      case None => controllers.myapis.update.routes.UpdateApiAddPrefixesController.onPageLoad(CheckMode).url
    }
