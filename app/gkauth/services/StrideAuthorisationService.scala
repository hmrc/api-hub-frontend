...
import gkauth.config.StrideAuthConfig
import gkauth.connectors.StrideAuthConnector
import gkauth.controllers.actions.ForbiddenHandler
import gkauth.domain.models.GatekeeperRoles.{ADMIN, SUPERUSER, USER}
import gkauth.domain.models.{GatekeeperRole, GatekeeperRoles, GatekeeperStrideRole, LoggedInRequest}
import gkauth.domain.models.GatekeeperRoles.{ADMIN, SUPERUSER, USER}
    ...
    authorise(strideRoleRequired) map {
      case Some(name) ~ authorisedEnrolments =>
        ...
        (
          ...
          case (true, _, _) => applyRole(ADMIN)
          case (_, true, _) => applyRole(SUPERUSER)
          case (_, _, true) => applyRole(USER)
