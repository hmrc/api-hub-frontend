...
trait ApproverOrApplicationAuthAction extends ActionRefiner[IdentifierRequest, ApplicationRequest]

trait ApproverOrApplicationAuthActionProvider {

  def apply(applicationId: String)(implicit ec: ExecutionContext): ApproverOrApplicationAuthAction

}

@Singleton
class ApproverOrApplicationAuthActionProviderImpl @Inject()(
  apiHubService: ApiHubService,
  errorResultBuilder: ErrorResultBuilder,
  override val messagesApi: MessagesApi
) extends ApproverOrApplicationAuthActionProvider with I18nSupport {

  def apply(applicationId: String)(implicit ec: ExecutionContext): ApproverOrApplicationAuthAction = {
    new ApproverOrApplicationAuthAction with FrontendHeaderCarrierProvider {
  ...
  private def isTeamMember(application: Application, user: UserModel): Boolean = {
    application.teamMembers.exists(teamMember => teamMember.email.equals(user.email))
  }
