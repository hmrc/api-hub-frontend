...
class UpdateApiSelectEgressController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      val preparedForm = request.userAnswers.get(UpdateApiSelectEgressPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        formValues =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UpdateApiSelectEgressPage, formValues.value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(UpdateApiSelectEgressPage, mode, updatedAnswers))
      )

  }

  private def buildView(status: Status, form: Form[?], mode: Mode)(implicit request: DataRequest[AnyContent])= {
    request.userAnswers.get(UpdateApiTeamPage) match {
      case Some(team) =>
        apiHubService.listEgressGateways(hipEnvironments.deployTo).map(egressGateways => {
          ...
          val viewModel = ProduceApiSelectEgressViewModel("myApis.update.selectegress.title", teamEgresses, controllers.myapis.update.routes.UpdateApiSelectEgressController.onSubmit(mode))
