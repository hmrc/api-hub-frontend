...
class ProduceApiEgressController @Inject()(
  ...
  def onSubmit(mode: Mode, next: String): Action[AnyContent] = identify {
    implicit request => {
      next match {
        case "hods" => Redirect(routes.ProduceApiShortDescriptionController.onPageLoad(mode))
        case "prefixes" => Redirect(routes.ProduceApiShortDescriptionController.onPageLoad(mode))
      }
    }
  }
}
