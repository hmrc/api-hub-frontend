...
class ProduceApiUploadWiremockController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = identify {
    implicit request => Ok(view(controllers.myapis.produce.routes.ProduceApiUploadWiremockController.onSubmit(mode)))
  }

  def onSubmit(mode: Mode): Action[AnyContent] = identify {
    implicit request => Redirect(controllers.myapis.produce.routes.ProduceApiEnterWiremockController.onPageLoad(mode))
  }
