  ...
  def onPageLoad(): Action[AnyContent] = optionallyIdentified.async {
    implicit request =>
      apiHubService.getApis().map {
        case apiDetails: Seq[ApiDetailSummary] @unchecked => Ok(view(request.user, apiDetails.sortWith( _.title.toUpperCase < _.title.toUpperCase), domains, hods, platforms))
